...
class Recipient(TimeStampedModel):
    ...
    ego = models.FloatField(
        _("Эго"), validators=[MinValueValidator(0), MaxValueValidator(10)], default=0.0
    )
...
class Bookmarks(TimeStampedModel):
    ...
    recipient = models.ForeignKey(
        "Recipient", on_delete=models.CASCADE, to_field="id", db_column="recipient_id"
    )
    movie = models.ForeignKey(
        "Movie", on_delete=models.CASCADE, to_field="id", db_column="movie_id"
    )
...
class Notification(TimeStampedModel):
    ...
    status = models.CharField(
        _("Статус"), max_length=20, choices=NotificationStatuses.choices
    )
...
class NotificationRecipient(TimeStampedModel):
    ...
    class Meta:
        indexes = [
            models.Index(
                fields=["recipient_id", "notification_id"],
...
class Scheduler(TimeStampedModel):
    ...
    when = models.CharField(
        _("Периодичность"), max_length=20, choices=Frequency.choices
    )
    priority = models.CharField(_("Приоритет"), max_length=20, choices=Priority.choices)
...
class SchedulerRecipient(TimeStampedModel):
    ...
    scheduler = models.ForeignKey(
        "Scheduler", on_delete=models.CASCADE, to_field="id", db_column="scheduler_id"
    )

    class Meta:
        indexes = [
            models.Index(
                fields=["scheduler_id", "recipient_id"], name="scheduler_id_recipient"
            )
        ]
