...
def init_db(app: Flask):
    ...
    with app.app_context():
        ...
        if not Role.query.filter_by(title="root").first():
            ...
            user1.save_to_db()
            Role(
                title="administrator", description="Роль для администратора."
            ).save_to_db()
            Role(
                title="subscriber", description="Роль для просмотра скрытого контента."
            ).save_to_db()
            Role(
                title="authenticated user",
                description="Роль для авторизованного пользователя.",
            ).save_to_db()
            Role(
                title="anonymous user", description="Роль для анонимного пользователя."
            ).save_to_db()
        if not User.query.filter_by(username="admin").first():
            super_user = User(
                username="admin",
                email="menergo@mail.ru",
                password="123123",
                status=True,
            )
