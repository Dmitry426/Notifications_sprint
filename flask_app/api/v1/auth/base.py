...
from flask_app.models.models import AuthHistory, Role, User
...
def generate_confirmation_token(user, email):
    ...
    confirm_token: str = create_access_token(
...
def get_token_social_data(login, email):
    ...
    if not check_user:
        ...
        check_user = User(
            username=login, email=email, password="ckbirjv_ckj;ysq_gfhjkm"
        )
        check_user.status = True
        check_user.roles.append(
            db.session.query(Role).filter_by(title="authenticated user").first()
        )
